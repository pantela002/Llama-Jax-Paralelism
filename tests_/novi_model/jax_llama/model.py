...
class FlaxLLaMAPreTrainedModel():
    ...
    def __init__(
        ...
        **kwargs,
        ...
        rngs = nnx.Rngs(seed)
        self.module = self.module_class(config=config, dtype=dtype, rngs=rngs, **kwargs)
...
class FlaxLLaMABlockCollection(nnx.Module):
    def __init__(self, config: LLaMAConfig, dtype=jnp.float32, param_dtype=jnp.float32, precision=None, rngs=None):
        super().__init__()
        self.blocks = [
            FlaxLLaMABlock(config=config, rngs=rngs)
...
class FlaxLLaMAForCausalLMModule(nnx.Module):
    def __init__(
        ...
        self.lm_head.kernel = jax.lax.with_sharding_constraint(
            self.lm_head.kernel,
